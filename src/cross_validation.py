...
# Ref. https://www.youtube.com/watch?v=2wQlD46eICE&t=821s
    ...
    def split(self):
        if self.problem_type in ("binary_classification", "multiclass_classification"):
            ...
            if unique_values == 1:
                ...
                for fold, (train_idx, val_idx) in enumerate(kf.split(X=self.dataframe, y=self.dataframe[target].values)): # pylint: disable=unused-variable
...
if __name__ == "__main__":
    df = pd.read_csv("../input/train_val.csv")
    cv = CrossValidation(df, shuffle=True, target_cols=["damage_grade"], 
                         problem_type="multiclass_classification", multilabel_delimiter=" ")
    df_split = cv.split()
    print(df_split.head())
    print(df_split.kfold.value_counts())
    # Added the file with kfold field
